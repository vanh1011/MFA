...
import { DeviceMobile, Key, Lifebuoy, Question, ShieldCheck, Timer } from '@phosphor-icons/react'
...
export function FaqAccordion({ sections }: { sections: Section[] }) {
  const icons = [ShieldCheck, DeviceMobile, Timer, Lifebuoy, Key]
          ...
          <AccordionItem value={section.id} key={section.id} id={section.id} className="faq-accordion-item">
            <AccordionTrigger className="faq-accordion-trigger">
              <span className="faq-question-heading">
                <span className="faq-question-icon"><ItemIcon weight="duotone" /></span>
                {section.title}
              </span>
            </AccordionTrigger>
